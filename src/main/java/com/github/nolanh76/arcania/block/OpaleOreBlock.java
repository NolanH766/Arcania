...
public class OpaleOreBlock extends ArcaniaModElements.ModElement {
	@ObjectHolder("arcania:opale_ore")
	public static final Block block = null;
	public OpaleOreBlock(ArcaniaModElements instance) {
		super(instance, 27);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("opale_ore", "opale_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 3)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(2, 4, 4, 32))));
		}
	}
}
