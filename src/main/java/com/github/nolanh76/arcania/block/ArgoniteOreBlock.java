...
public class ArgoniteOreBlock extends ArcaniaModElements.ModElement {
	@ObjectHolder("arcania:argonite_ore")
	public static final Block block = null;
	public ArgoniteOreBlock(ArcaniaModElements instance) {
		super(instance, 24);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("argonite_ore", "argonite_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 8)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(8, 1, 1, 16))));
		}
	}
}
